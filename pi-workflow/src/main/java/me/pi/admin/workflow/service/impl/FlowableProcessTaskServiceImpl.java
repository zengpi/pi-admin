...
import me.pi.admin.workflow.service.FlowableProcessCopyService;
...
public class FlowableProcessTaskServiceImpl implements FlowableProcessTaskService {
    /**
     * 日期范围长度
     */
    ...
    private final FlowableProcessCopyService flowableProcessCopyService;
    private final RuntimeService runtimeService;

    @Override
    public List<? extends TaskInfo> listTasks(FlowableTaskQuery query) {
        Assert.notNull(query, "参数不能为空");

        TaskInfoQueryWrapper taskInfoQueryWrapper;
        if (Objects.nonNull(query.getIsFinished()) && Boolean.TRUE.equals(query.getIsFinished())) {
        ...
        if (Objects.nonNull(query.getProcessInstanceId())) {
            taskInfoQueryWrapper.getTaskInfoQuery().processInstanceId(query.getProcessInstanceId());
        }

        return taskInfoQueryWrapper.getTaskInfoQuery().list();
    }

    @Override
    public List<ProcessTaskVO> listTaskByProcessInstanceId(String processInstanceId) {
        HistoricTaskInstanceQuery historicTaskInstanceQuery = historyService.createHistoricTaskInstanceQuery();
        historicTaskInstanceQuery.finished();
        TaskInfoQueryWrapper taskInfoQueryWrapper = new TaskInfoQueryWrapper(historicTaskInstanceQuery);
        List<? extends TaskInfo> list = taskInfoQueryWrapper
                .getTaskInfoQuery()
    ...
    @Override
    public IPage<TodoTaskVO> listTodoTask(TodoTaskQuery todoTaskQuery) {
    ...
    @Override
    public ProcessFormVO getAllTaskFormByTaskId(String taskId) {
        ...
        // 系统表单
        ActReForm form = flowableProcessFormService.getByFormKey(historicTaskInstance.getFormKey());
        ...
        if (Objects.nonNull(form)) {
            ...
            for (Map.Entry<String, Object> entry : historicTaskInstance.getTaskLocalVariables().entrySet()) {
        ...
        return new ProcessFormVO(taskFormModel, simpleFormModel);
    }

    @Override
    public FormInfo getTaskFormByTaskId(String taskId) {
    ...
    public void approve(ApproveCompleteDTO approveCompleteDto) {
        ...
        if (DelegationState.PENDING.equals(task.getDelegationState())) {
            ...
            if (CollUtil.isNotEmpty(approveCompleteDto.getVariables())) {
                if (Objects.isNull(approveCompleteDto.getFormId())) {
                    taskService.complete(approveCompleteDto.getTaskId(), approveCompleteDto.getVariables());
                } else {
                    taskService.completeTaskWithForm(approveCompleteDto.getTaskId(),
                            approveCompleteDto.getFormId(), approveCompleteDto.getOutcome(),
                            approveCompleteDto.getVariables());
                }
    ...
    @Override
    public IPage<DoneTaskVO> listDoneTask(TodoTaskQuery todoTaskQuery) {
        ...
        if (StringUtils.hasText(todoTaskQuery.getProcessDefinitionName())) {
            historicTaskInstanceQuery.processDefinitionNameLike("%" + todoTaskQuery.getProcessDefinitionName() + "%");
        }
        if (Objects.nonNull(todoTaskQuery.getDateRange()) && todoTaskQuery.getDateRange().length == DATE_RANGE_LENGTH) {
            historicTaskInstanceQuery.taskCompletedAfter(DateUtil.localDateToDate(todoTaskQuery.getDateRange()[0]));
            historicTaskInstanceQuery.taskCompletedBefore(DateUtil.localDateToDate(todoTaskQuery.getDateRange()[1]));
        }
        ...
        List<HistoricTaskInstance> historicTaskInstances = historicTaskInstanceQuery
                .listPage(offset, todoTaskQuery.getPageSize());
    ...
    @Override
    public IPage<CopyVO> listCopies(TodoTaskQuery todoTaskQuery) {
        LambdaQueryWrapper<ActHiCopy> wrapper = Wrappers.lambdaQuery(ActHiCopy.class)
                .like(StringUtils.hasText(todoTaskQuery.getProcessDefinitionName()), ActHiCopy::getProcessDefinitionName,
                        todoTaskQuery.getProcessDefinitionName())
                .eq(ActHiCopy::getCopyUser, SecurityUtils.getUserName());

        if (Objects.nonNull(todoTaskQuery.getDateRange())) {
            if (todoTaskQuery.getDateRange().length != DATE_RANGE_LENGTH) {
                throw new BadRequestException(ResponseStatusEnum.REQUEST_PARAM_ERROR,
                        "如果需要按时间搜索，请指定开始时间和结束时间");
            }
            wrapper.ge(ActHiCopy::getCreateTime, todoTaskQuery.getDateRange()[0])
                    .le(ActHiCopy::getCreateTime, todoTaskQuery.getDateRange()[1]);
        }

        IPage<ActHiCopy> page = flowableProcessCopyService.page(todoTaskQuery.page(), wrapper);
    ...
    private void setCopyUsers(Task task, String processInstanceId, List<String> copyUsers) {
        if (!copyUsers.isEmpty()) {
            ...
            flowableProcessCopyService.saveBatch(copies);
        }
    }
