...
public class ProcessFormController {
    ...
    public ResponseData<IPage<ProcessDefinitionStartFormVO>> listForms(BaseQuery baseQuery) {
        IPage<ActReForm> forms = flowableProcessFormService.listForms(baseQuery);
    ...
    public ResponseData<?> deleteForm(@PathVariable Set<Long> ids) {
        flowableProcessFormService.deleteForms(ids);
        return ResponseData.ok();
    }

    @GetMapping("/{formId}")
    @Operation(summary = "根据表单 ID 获取表单")
    public ResponseData<ProcessDefinitionStartFormVO> getById(@PathVariable String formId) {
        return ResponseData.ok(processFormConverter.poToVo(flowableProcessFormService.getById(formId)));
    }

    @GetMapping("/allForms")
    @Operation(summary = "获取所有表单")
    public ResponseData<List<ProcessDefinitionStartFormVO>> listAllForms() {
        return ResponseData.ok(flowableProcessFormService.listAllForms());
    }
    ...
    private void saveOrUpdate(FormDTO dto) {
        ...
        flowableProcessFormService.saveOrUpdate(dto);
    }
}
