...
public class ProcessDefinitionController {
    ...
    public ResponseData<IPage<ProcessDefinitionVO>> listProcessDefinitions(ProcessDefQuery processDefQuery) {
        return ResponseData.ok(flowableProcessDefinitionService.listProcessDefinitions(processDefQuery));
    }
    ...
    public ResponseData<IPage<ProcessDefinitionVO>> getBootableDefinition(BootableDefinitionQuery query) {
        return ResponseData.ok(flowableProcessDefinitionService.getBootableDefinition(query));
    }

    @GetMapping("/{processDefinitionId}/start-form")
    @Operation(summary = "获取启动流程表单")
    public ResponseData<ProcessDefinitionStartFormVO> getProcessDefinitionStartForm(@PathVariable String processDefinitionId) {
        return ResponseData.ok(flowableProcessDefinitionService.getProcessDefinitionStartForm(processDefinitionId));
    }
    ...
    public ResponseData<String> getBpmnXml(@PathVariable String definitionId) {
        return ResponseData.ok(flowableProcessDefinitionService.getBpmnXml(definitionId));
    }
    ...
    public ResponseData<?> changeState(@PathVariable String definitionId, @PathVariable Boolean suspended) {
        flowableProcessDefinitionService.changeState(definitionId, suspended);
    ...
    public ResponseData<IPage<ProcessDefinitionVO>> listHistoryProcessDefinitions(ProcessDefQuery processDefQuery) {
        return ResponseData.ok(flowableProcessDefinitionService.listHistoryProcessDefinitions(processDefQuery));
    }
}
