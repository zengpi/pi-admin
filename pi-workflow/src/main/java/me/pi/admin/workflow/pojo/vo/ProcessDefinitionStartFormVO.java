...
@Data
@NoArgsConstructor
@Schema(title = "启动流程表单视图对象")
public class ProcessDefinitionStartFormVO {
    /**
     * 唯一标识
     */
    ...
    public ProcessDefinitionStartFormVO(FormInfo formInfo) {
        // 非内置表单
        this.builtIn = 0;
        this.id = formInfo.getId();
        this.name = formInfo.getName();
        this.description = formInfo.getDescription();
        this.formKey = formInfo.getKey();
        this.version = formInfo.getVersion();
    }

    public ProcessDefinitionStartFormVO(FormInfo formInfo, SimpleFormModel formModel) {
        this(formInfo);
